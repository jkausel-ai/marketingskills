#!/bin/bash
# opus-delegate.sh — DIRECTOR TIER (Sonnet 4.6 CLI, $0 via Max)
...
' < "$TOKEN_SECRET_FILE")"
        ...
        echo "DELEGATE_FAIL:opus:missing_oauth_token"
        exit 1
    fi
fi
...
PROMPT="${1:-}"
if [ -z "$PROMPT" ]; then
    echo "DELEGATE_FAIL:opus:no_prompt"
    exit 1
fi
...
echo "[$TS] OPUS-DELEGATE START: $(head -c 120 "$PROMPT_FILE")..." >> "$LOG"
...
OUTPUT=$(timeout "$TIMEOUT" /root/.local/bin/claude -p \
    ...
    EXIT_CODE=$?
    echo "[$TS] OPUS-DELEGATE FAIL: exit=$EXIT_CODE" >> "$LOG"
    $CLEANUP_FILE && rm -f "$PROMPT_FILE"
    echo "DELEGATE_FAIL:opus:exit=$EXIT_CODE"
    exit $EXIT_CODE
}
...
if [ "$OUTPUT_LEN" -lt "$MIN_OUTPUT_CHARS" ]; then
    echo "[$TS] OPUS-DELEGATE WARN: short ($OUTPUT_LEN chars)" >> "$LOG"
fi

echo "[$TS] OPUS-DELEGATE DONE: ${OUTPUT_LEN} chars" >> "$LOG"
echo "$OUTPUT"
